...
async function calcularCierre(desde: Date, hasta: Date) {
  const [r] = await db.execute(sql`
    ...
    WHERE COALESCE(fecha_movimiento, created_at) BETWEEN ${desde.toISOString()} AND ${hasta.toISOString()}
  ...
  const ordenes = await db.execute(sql`
    ...
    FROM ordenes
    WHERE fecha_ingreso BETWEEN ${desde.toISOString()} AND ${hasta.toISOString()}
      AND es_presupuesto = false
  `) as unknown as { total: number; facturado: number }[];

  const cats = await db.execute(sql`
    SELECT categoria, COUNT(*)::INT AS cnt, COALESCE(SUM(CAST(total AS NUMERIC)), 0)::FLOAT AS total
    FROM caja_movimientos
    WHERE COALESCE(fecha_movimiento, created_at) BETWEEN ${desde.toISOString()} AND ${hasta.toISOString()}
      AND tipo='egreso' AND categoria IS NOT NULL
    GROUP BY categoria ORDER BY total ASC LIMIT 5
...
function buildReport(periodo: Periodo, desde: Date, hasta: Date, data: Awaited<ReturnType<typeof calcularCierre>>) {
  ...
  if (data.topGastos.length > 0) {
    txt += `\n🛒 Top gastos:\n`;
    for (const c of data.topGastos) {
      txt += `  · ${c.categoria}: $${fmt(Math.abs(c.total))}\n`;
    }
  }
  return txt;
}
...
export async function POST(req: Request) {
  ...
  if (periodo === 'diario') {
    ...
    // mensual: del día N del mes pasado al día N del mes actual (corte default 5)
    const corteStr = await getConfig('cierre_dia_corte');
    const corte = Number(corteStr) || 5;
  ...
  const data = await calcularCierre(desde, hasta);
  const text = buildReport(periodo, desde, hasta, data);
  const result = await notificar('cierre', text);

  return NextResponse.json({ ok: true, periodo, desde: desde.toISOString(), hasta: hasta.toISOString(), data, text, notif: result });
}
